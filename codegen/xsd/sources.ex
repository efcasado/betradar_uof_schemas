defmodule Mix.UOF.Schemas.XSD.Sources do
  @moduledoc false
  ...
  # `mix uof.schemas.gen` and are never vendored into the repo (see
  # `.gitignore`). The public schema page is deliberately NOT used: it is
  # incomplete (no cashout/probability schema) and not tag-pinned. The Java SDK
  # is likewise not used: it lacks the `bsa` sports API and cashout schemas.
  ...
  @groups [:custombet, :unifiedfeed, :sports_api, :probability, :feed]
  ...
  @doc """
  Download a group's XSDs from the pinned SDK tag into `dir(group)`, returning
  the directory. Only invoked for an empty cache (see `mix uof.schemas.gen`);
  run `make clean` to clear `priv/xsd/` before forcing a fresh download.
  """
  ...
  defp ua, do: [{"user-agent", "betradar_uof_schemas-xsd-codegen"}]
end
